...
export type WelcomeMessage = Envelope<'welcome', { yourId: string; roomCode: string; players: string[] }>;
...
export type InputMessage = Envelope<'input', {
  thrust: number;
  strafe: number;
...
export type SnapshotMessage = Envelope<'snapshot', {
  tick: number;
